...
class Helper:
    def __init__(self, db_path: Optional[str] = None):
        ...
        self._db_path = db_path or os.path.join(project_root, "SQL", "helper.db")
...
""", (date, amt, amt))
    ...
    def save_pokemon(self, user_id: int, pokemon: str, amount: int, caught: bool = True, image: Optional[str] = None):
        with sqlite3.connect(self._db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("""
...
""", (str(user_id), rounded, rounded))
    ...
    def get_money(self, user_id: int) -> float:
        with sqlite3.connect(self._db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT Money FROM GamblingCache WHERE UserID = ?", (str(user_id),))
            result = cursor.fetchone()
...
""", (user_id, f"%{name}%"))
    ...
    def get_girl(self, user_id: int) -> List[Tuple[str, str, str]]:
        with sqlite3.connect(self._db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT GirlName, GirlInfo, GirlImg FROM GirlsCache WHERE UserID = ?", (user_id,))
