...
class AIGroup(app_commands.Group):
    ...
    #             prompt = prompt
    #             input_image = load_image(image.url)

    #             image = pipe(image=input_image, prompt=prompt).images[0]
    #             print(image)
    #             # Step 1: Download the image
    #             data = self.cog.download_with_retries(image)
    ...
    async def list_characters(self, interaction: discord.Interaction):
        ...
        all_characters = self._db.get_girl(interaction.user.id)
        all_characters.sort()
        if not all_characters:
            await interaction.followup.send(
                "You haven't created any characters yet. Use `/ai create` to get started!"
            )
            return
    ...
    async def talk_to_character(
        ...
        # Build embed
        embed = discord.Embed(
            title=character_name, description=reply, color=discord.Color.from_rgb(120, 220, 212)
        )
